...
## Active Hazard Lifecycle Display

- PR #346 adds display-only active hazard lifecycle and player HUD state for session-local active/held/dismissed hazard candidate-control results. It still does not apply station effects, execute response actions, clear hazards, apply consequences, persist settings, or mutate Foundry documents.
...
2. **Permanent card schema planning**
   ...
   - PR #337 provides a validation-only adapter so builder/importer/dev paths can recognize and preview v0 card packs without runtime consumption or a builder rewrite.
