...
- Current implementation branch: `rebuild/arcflight-voyage-events-alpha`.
- Target Foundry VTT version: v14.
- Target game system: Pathfinder Second Edition (PF2e).
- Read `docs/voyage-event-v3-decisions.md` before any Voyage Event work.
- The full accepted alpha-scope archive remains on `rebuild/arcflight-gameplay-v3` at `docs/voyage-event-alpha-scope.md`.
...
- Station degree values are:
  ...
  - Success: `+1`
  - Critical Success: `+2`
- Round and event result matrices must follow the accepted design decisions.
...
- Reuse existing ship flags, station assignments, PF2e vehicle architecture, socket support, sheet-registration patterns, and helper conventions where compatible.
...
- The five Voyage Event stations are an active subset of the shared station registry.
- Voyage Event Pressure is event-local and must not overwrite refit-pressure data.
...
- Prefer a clear incomplete result over silently inventing rules that are not in the accepted design.
