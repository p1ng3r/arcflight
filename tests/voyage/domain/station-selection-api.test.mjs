...
test("registers station-selection editing helpers on public and devTools APIs", () => {
  ...
  try {
    ...
    for (const helper of [validateVoyageEncounterRiskBids, applyVoyageEncounterRiskBidSelection, applyVoyageEncounterRiskBidChange, applyVoyageEncounterRiskBidClear, applyVoyageEncounterPendingCheckResult, prepareVoyageEncounterResolutionCompletion, applyVoyageEncounterConsequencesTransition, validateVoyageEncounterActionOutcomeDefinitions, analyzeVoyageEncounterActionOutcomeDefinitions]) { assert.equal(result[helper.name], helper); assert.equal(result.devTools[helper.name], helper); }
...
test("station-selection editing API module exposes named helpers", () => {
  ...
  for (const helper of [applyVoyageEncounterPendingCheckResult, prepareVoyageEncounterResolutionCompletion, applyVoyageEncounterConsequencesTransition, validateVoyageEncounterActionOutcomeDefinitions, analyzeVoyageEncounterActionOutcomeDefinitions]) assert.equal(typeof helper, "function");
  ...
  assert.equal(typeof registerVoyageStationSelectionEditingApi, "function"); assert.equal(typeof validateVoyageEncounterActionOutcomeDefinitions, "function"); assert.equal(typeof analyzeVoyageEncounterActionOutcomeDefinitions, "function");
});
