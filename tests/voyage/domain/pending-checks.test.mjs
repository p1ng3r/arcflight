...
test("resolved result getter is read exactly once", () => { const value = state(); const prepared = applyVoyageEncounterPendingCheckPreparation(value, { pendingCheckIds: [{ sequence: 0, pendingCheckId: "check" }] }).nextState; let reads = 0; const result = { total: 20, degreeOfSuccess: 2, degreeOfSuccessSlug: "success", statisticSlug: "diplomacy", dc: 20, rollMode: "public" }; Object.defineProperty(prepared.pendingChecks[0], "result", { enumerable: true, get() { reads += 1; return result; } }); prepared.pendingChecks[0].status = "resolved"; assert.equal(validateVoyageEncounterPendingChecks(prepared).valid, true); assert.equal(reads, 1); });
test("resolved result does not perform a second getter read", () => { const value = state(); const prepared = applyVoyageEncounterPendingCheckPreparation(value, { pendingCheckIds: [{ sequence: 0, pendingCheckId: "check" }] }).nextState; let reads = 0; const result = { total: 20, degreeOfSuccess: 2, degreeOfSuccessSlug: "success", statisticSlug: "diplomacy", dc: 20, rollMode: "public" }; Object.defineProperty(prepared.pendingChecks[0], "result", { enumerable: true, get() { reads += 1; if (reads > 1) throw new Error("second read"); return result; } }); prepared.pendingChecks[0].status = "resolved"; assert.equal(validateVoyageEncounterPendingChecks(prepared).valid, true); assert.equal(reads, 1); });
