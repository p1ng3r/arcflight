...
test("reports unresolved prepared checks and completed resolution", () => { const before = state(); assert.equal(prepareVoyageEncounterResolutionCompletion(before).readyForConsequences, false); const after = applyVoyageEncounterPendingCheckResult(before, rolled()).nextState; const report = prepareVoyageEncounterResolutionCompletion(after); assert.equal(report.readyForConsequences, true); assert.equal(report.unresolvedCheckCount, 0); assert.equal(report.resolvedCheckCount, 1); });
