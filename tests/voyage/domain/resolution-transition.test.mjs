...
test("permits a valid empty locked plan", () => { const state = lockedState(); state.availableStations = []; state.selections = {}; const result = applyVoyageEncounterResolutionTransition(state, { phaseStartSnapshotId: "empty" }); assert.equal(result.ok, true); assert.deepEqual(result.events[0].orderedActions, []); });
