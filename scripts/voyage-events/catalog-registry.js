...
function validateGroupEntry(group, entry, errors, path) {
  ...
  if (group === "rewards" && (!['plus2', 'plus5', 'plus8'].includes(entry.bidBand) || !stationArray("sourceStations") || !stationArray("targetStations") || !Array.isArray(entry.actionTags) || !entry.actionTags.every((tag) => typeof tag === "string" && tag.trim()))) issue(errors, "error", "catalog.reward.invalid", path, "Reward requires a plus2, plus5, or plus8 bid band, active source and target stations, and non-empty action tags.", entry.id);
