...
export async function runFrameworkSmokeTest(options = {}) {
  ...
  try {
    ...
    check(result, "Travel Event Builder published search input avoids immediate full rerender cursor jump", /#schedulePublishedSearchRender/.test(travelEventBuilderUiSource) && /key === "librarySearchText" && event\.type === "input"/.test(travelEventBuilderUiSource) && /return;\s*}\s*if \(this\.#publishedSearchRenderTimeout\)/.test(travelEventBuilderUiSource) && /setSelectionRange/.test(travelEventBuilderUiSource), "published search input source", "search input is debounced and restores caret instead of using immediate select/change rerender path");
