import {
  ...
  renderPublishedTravelEventImportPreviewHtml
...
export async function runFrameworkSmokeTest(options = {}) {
  ...
  try {
    ...
    const stationActor = { id: "crew-a", uuid: "Actor.crew-a", name: "Aster Vale", type: "character", system: { skills: { diplomacy: { mod: 7 } }, perception: { mod: 9 } } };
    const vehicleActor = { id: "ship-a", uuid: "Actor.ship-a", name: "The Arc", type: "vehicle", getFlag: (scope, key) => scope === ARCFLIGHT_MODULE_ID && key === "system" ? { stations: { assignments: { navigator: { stationKey: "navigator", assigneeType: "actor", actorId: "crew-a", actorUuid: "Actor.crew-a", name: "Aster Vale" }, watchmaster: { stationKey: "watchmaster", assigneeType: "actor", actorId: "crew-a", actorUuid: "Actor.crew-a", name: "Aster Vale" } } } } : null };
    const assignmentActors = [stationActor, { id: "crew-b", uuid: "Actor.crew-b", name: "Brindle", type: "npc" }, vehicleActor];
    const assignmentExportsExist = [getTravelEventRunnerStationActorOptions, normalizeTravelEventRunnerStationAssignments, prepareTravelEventRunnerStationAssignmentState, updateTravelEventRunnerStationAssignment, clearTravelEventRunnerStationAssignment, resetTravelEventRunnerStationAssignmentToShip, getTravelEventRunnerShipStationAssignments].every((helper) => typeof helper === "function")
    ...
    const assignmentHelperSource = [getTravelEventRunnerStationActorOptions, normalizeTravelEventRunnerStationAssignments, prepareTravelEventRunnerStationAssignmentState, updateTravelEventRunnerStationAssignment, clearTravelEventRunnerStationAssignment, resetTravelEventRunnerStationAssignmentToShip, getTravelEventRunnerShipStationAssignments].map((helper) => String(helper)).join("\n");
    ...
    const shipSeedAssignments = getTravelEventRunnerShipStationAssignments(vehicleActor);
    check(result, "Ship-seeded station assignment snapshot reads selected ship station data", shipSeedAssignments.navigator.source === "ship" && shipSeedAssignments.navigator.overridden === false && shipSeedAssignments.navigator.actorId === stationActor.id && shipSeedAssignments.engineer.source === "empty", "ship seed", shipSeedAssignments);
    const normalizedShipSeedSession = createTravelEventRunnerSession(loadedRunnerFixtureEvent.event, { ship: { actorId: vehicleActor.id, actorUuid: vehicleActor.uuid, actorName: vehicleActor.name, actorType: vehicleActor.type }, actors: assignmentActors });
    check(result, "createTravelEventRunnerSession resolves normalized selected ship metadata back to an Actor for seeding", normalizedShipSeedSession.session?.stationAssignments?.navigator?.source === "ship" && normalizedShipSeedSession.session?.stationAssignments?.navigator?.actorName === stationActor.name, "normalized ship seed", normalizedShipSeedSession.session?.stationAssignments?.navigator);
    const pathwaySelectedStart = await startTravelEventRunnerFromPublishedEvent(runnerFixtureKey, { library: runnerFixturePublishedLibrary, actors: assignmentActors, actorId: vehicleActor.id });
    check(result, "Published Start Run pathway uses the UI-selected ship actor for station seeding without a controlled token", pathwaySelectedStart.ok === true && pathwaySelectedStart.session?.stationAssignments?.navigator?.source === "ship" && pathwaySelectedStart.session?.stationAssignments?.watchmaster?.actorName === stationActor.name, "pathway seeded", pathwaySelectedStart.session?.stationAssignments);
    const fallbackStart = await startTravelEventRunnerFromPublishedEvent(runnerFixtureKey, { library: runnerFixturePublishedLibrary, actors: assignmentActors });
    check(result, "Published Start Run pathway still falls back to the default selected ship option", fallbackStart.ok === true && fallbackStart.session?.stationAssignments?.navigator?.source === "ship", "fallback seeded", fallbackStart.session?.stationAssignments?.navigator);
    const otherVehicleActor = { id: "ship-b", uuid: "Actor.ship-b", name: "Wrong Ship", type: "vehicle", getFlag: (scope, key) => scope === ARCFLIGHT_MODULE_ID && key === "system" ? { stations: { assignments: { navigator: { stationKey: "navigator", assigneeType: "actor", actorId: "crew-b", actorUuid: "Actor.crew-b", name: "Brindle" } } } } : null };
    const selectedPriorityStart = await startTravelEventRunnerFromPublishedEvent(runnerFixtureKey, { library: runnerFixturePublishedLibrary, actors: [otherVehicleActor, ...assignmentActors], actorId: vehicleActor.id });
    check(result, "Selected ship id takes priority over other available ship fallback options", selectedPriorityStart.ok === true && selectedPriorityStart.session?.stationAssignments?.navigator?.actorName === stationActor.name, "selected priority", selectedPriorityStart.session?.stationAssignments?.navigator);

    const activeRunnerSessionCreated = createTravelEventRunnerSession(loadedRunnerFixtureEvent.event, { ship: vehicleActor });
    ...
    check(result, "Station assignment state returns exactly Travel Five safe rows", emptyAssignmentState.rows.length === 5 && emptyAssignmentState.rows.some((row) => row.stationKey === "navigator" && row.sourceLabel === "Ship") && emptyAssignmentState.rows.some((row) => row.stationKey === "engineer" && row.assignedActorName === "Unassigned"), "five ship-seeded rows", emptyAssignmentState.rows);
    ...
    check(result, "Clearing Navigator assignment succeeds", clearedNavigator.ok === true && clearedNavigator.session.stationAssignments.navigator.actorId === "" && clearedNavigator.session.stationAssignments.navigator.actorName === "" && clearedNavigator.session.stationAssignments.navigator.source === "override" && clearedNavigator.session.stationAssignments.navigator.overridden === true, "cleared", clearedNavigator.session.stationAssignments.navigator);
    ...
    const resetNavigator = resetTravelEventRunnerStationAssignmentToShip(clearedNavigator.session, "navigator", { ship: vehicleActor });
    check(result, "Reset to ship assignment restores one station from the selected ship snapshot", resetNavigator.ok === true && resetNavigator.session.stationAssignments.navigator.source === "ship" && resetNavigator.session.stationAssignments.navigator.actorId === stationActor.id, "reset", resetNavigator.session.stationAssignments.navigator);
    check(result, "Runner override and reset do not mutate ship assignment source", vehicleActor.getFlag(ARCFLIGHT_MODULE_ID, "system").stations.assignments.navigator.actorId === stationActor.id, "ship source unchanged", vehicleActor.getFlag(ARCFLIGHT_MODULE_ID, "system").stations.assignments.navigator);
    ...
    const perceptionSession = cloneTravelEventRunnerSession(activeRunnerSession).session;
    perceptionSession.event.rounds[0].activeStations = ["watchmaster"];
    perceptionSession.event.rounds[0].stationPrompts = { watchmaster: { stationKey: "watchmaster", suggestedSkills: ["perception"], playerAction: "Keep watch." } };
    perceptionSession.roundResults[0].stationResults = { watchmaster: null };
    perceptionSession.stationAssignments.watchmaster = { actorId: stationActor.id, actorUuid: stationActor.uuid, actorName: stationActor.name, actorType: stationActor.type, source: "ship", overridden: false };
    const perceptionUiState = prepareTravelEventRunnerState(perceptionSession, { actors: assignmentActors });
    check(result, "Watchmaster Perception resolver returns a usable label for mock PF2E perception data", perceptionUiState.stations.some((row) => row.stationKey === "watchmaster" && row.statisticLabel.includes("+9")), "perception label", perceptionUiState.stations.find((row) => row.stationKey === "watchmaster")?.statisticLabel);
    ...
    const publishedLibraryStateHazardDescriptionSearch = preparePublishedTravelEventLibraryState({ library: publishedLibrarySearchFixture, librarySearchText: "hazard description" });
    ...
    const runnerUiSource = String(ArcflightTravelEventRunner);
    ...
    runnerHistoricalSession.stationAssignments.navigator = { actorId: "crew-a", actorUuid: "Actor.crew-a", actorName: "Aster Vale", actorType: "character", source: "override", overridden: true };
    ...
    check(result, "Travel Event Builder preserves scroll across forced rerenders", /#pendingScrollState/.test(travelEventBuilderUiSource) && /#captureScrollPosition/.test(travelEventBuilderUiSource) && /#restoreScrollPosition/.test(travelEventBuilderUiSource) && /requestAnimationFrame\(\(\) => requestAnimationFrame\(restore\)\)/.test(travelEventBuilderUiSource) && /scrollSelector/.test(travelEventBuilderUiSource) && /scrollTop/.test(travelEventBuilderUiSource), "scroll preservation source", "capture and double-rAF restore present");
    check(result, "Travel Event Builder published search input avoids full rerender while typing", !/#schedulePublishedSearchRender/.test(travelEventBuilderUiSource) && /key === "librarySearchText" && event\.type === "input"\) return;/.test(travelEventBuilderUiSource) && /#onBuilderKeydown/.test(travelEventBuilderUiSource) && /event\.key !== "Enter"/.test(travelEventBuilderUiSource), "published search input source", "search input updates state only on input; Enter/change use the explicit rerender path");
    ...
    check(result, "Published Travel Event import preview HTML escapes unsafe content", /&lt;script&gt;key&lt;\/script&gt;/.test(unsafePublishedPreviewHtml) && /&lt;script&gt;warning&lt;\/script&gt;/.test(unsafePublishedPreviewHtml) && /&lt;img src=x onerror=&quot;alert\(1\)&quot;&gt;/.test(unsafePublishedPreviewHtml) && !/<script>|<img|<svg|<em>|<strong>mode/.test(unsafePublishedPreviewHtml), "escaped preview html", unsafePublishedPreviewHtml);
    ...
    check(result, "Published export UI uses Foundry v13 saveDataToFile namespace", !/globalThis\.saveDataToFile/.test(travelEventBuilderUiSource) && /foundry\?\.utils\?\.saveDataToFile/.test(travelEventBuilderUiSource), "foundry.utils.saveDataToFile", "no deprecated global saveDataToFile access");
    check(result, "Travel Event Runner export UI uses Foundry v13 saveDataToFile namespace", !/globalThis\.saveDataToFile/.test(runnerUiSource) && /foundry\?\.utils\?\.saveDataToFile/.test(runnerUiSource), "foundry.utils.saveDataToFile", "no deprecated global saveDataToFile access");
    ...
    check(result, "Travel Event Runner UI state exposes import and disabled export without a session", emptyRunnerState.canImportSession === true && emptyRunnerState.canExportSession === false, "empty runner actions", { canImportSession: emptyRunnerState.canImportSession, canExportSession: emptyRunnerState.canExportSession });
    ...
    check(result, "Travel Event Runner UI state exposes export for active sessions", runnerRoundState.canImportSession === true && runnerRoundState.canExportSession === true && runnerRoundState.summary === null, "active runner actions", { canImportSession: runnerRoundState.canImportSession, canExportSession: runnerRoundState.canExportSession, status: runnerRoundState.session?.status });
    check(result, "Travel Event Runner UI source exposes collapsible session action menu", /sessionActionsExpanded/.test(runnerUiSource) && /data-arcflight-runner-toggle-session-actions/.test(runnerUiSource) && /data-arcflight-runner-export/.test(runnerUiSource) && /data-arcflight-runner-import-session/.test(runnerUiSource), "session action menu source", "toggle and import/export actions present");
    check(result, "Travel Event Runner local start source uses ship selection dialog and shared Start Run helper", /preparePublishedTravelEventRunnerLaunchState/.test(runnerUiSource) && /startTravelEventRunnerFromPublishedEvent/.test(runnerUiSource) && /Ship \/ PF2E vehicle/.test(runnerUiSource) && /#getControlledShipActorFallback/.test(runnerUiSource), "runner local start ship selector", "dialog and controlled-token fallback present");
    check(result, "Travel Event Runner UI source avoids browser prompt alert confirm APIs", !/(^|[^.])\b(prompt|alert|confirm)\s*\(/.test(runnerUiSource), "no browser prompt/alert/confirm", "DialogV2-only confirmations");
    ...
    check(result, "Travel Event Runner explicit import overwrite writes only Runner Session Library", runnerOverwriteImport.ok === true && runnerOverwriteImport.importMode === "overwrite" && runnerOverwriteImport.overwritten === true && runnerOverwriteImport.entry?.key === "smoke-import-session" && Object.keys(runnerOverwriteImport.library?.sessions ?? {}).length === 1 && runnerOverwriteImport.session?.status === "completed" && runnerOverwriteImport.session?.summary?.stagedProposedEffects?.length === 1 && runnerOverwriteImport.session?.appliedEffects?.records?.[0]?.undone === true && runnerOverwriteImport.session?.stationAssignments?.navigator?.source === "override" && runnerOverwriteImport.session?.appliedEffects?.records?.[0]?.undoReason === "smoke undo", "overwrite import", runnerOverwriteImport.entry);
    check(result, "Imported completed runner session loads with staged/applied/undone history visible and unapplied", runnerImportedLoaded.ok === true && runnerImportedLoaded.session?.status === "completed" && runnerImportedLoaded.session?.summary?.stagedProposedEffects?.length === 1 && runnerImportedLoaded.session?.appliedEffects?.records?.[0]?.undone === true && runnerImportedLoaded.session?.stationAssignments?.navigator?.source === "override", "loaded imported session", runnerImportedLoaded.session);
