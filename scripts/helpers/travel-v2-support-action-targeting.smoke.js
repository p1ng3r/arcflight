...
export async function runTravelV2SupportActionTargetingSmokeChecks() {
  ...
  try {
    ...
    assertSmoke(!session.supportAssistRecords && !session.travelV2SupportAssistRecords, "Support assist records are not created");
  ...
  return { ok: true, checked: ["action-constant", "normalization", "target-required", "self-target-rejection", "inactive-target-rejection", "valid-target-storage", "session-normalization", "objective-separation", "no-assist-or-backlash-records", "no-momentum-award", "no-side-effects"] };
}
