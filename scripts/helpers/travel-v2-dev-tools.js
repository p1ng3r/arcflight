...
export function prepareTravelV2EndOfEventResolutionDialogState(session, options = {}) {
  ...
  return Object.freeze({ version: TRAVEL_V2_DEV_TOOLS_VERSION, title: "End-of-Event Resolution", tabs: ["vignette", "rounds", "rewards", "consequences", "shipChanges", "followUps", "debug"], eventName: normalized?.event?.name ?? normalized?.event?.title ?? "Travel Event", shipName: normalized?.ship?.name ?? "", completedAt: normalized?.completedAt ?? "", outcomePackage: outcome, followUps, debugReport, applyButtonsExplicit: true });
}
...
export function prepareTravelV2CompletedSessionHistoryState(libraryStateOrSessions = {}, options = {}) {
  ...
  const rows = entries.map((entry) => ({ entry, session: entry.session ?? entry })).filter(({ session }) => session?.status === "completed" || session?.completed === true || session?.completedAt).map(({ entry, session }) => {
    ...
    return { key: entry.key ?? session.key ?? "", eventName: entry.eventName ?? session.event?.name ?? session.event?.title ?? "Travel Event", shipName: entry.shipName ?? session.ship?.name ?? "", outcome: outcome.eventOutcomeLabel ?? "", outcomeKey: outcome.eventOutcomeKey ?? "", completedAt: session.completedAt ?? entry.completedAt ?? "", completedAtLabel: session.completedAt ? new Date(session.completedAt).toLocaleString() : "", appliedAt: session.travelV2EventOutcomeApplication?.appliedAt ?? session.travelV2ActorApplication?.appliedAt ?? "", appliedAtLabel: session.travelV2EventOutcomeApplication?.appliedAt ? new Date(session.travelV2EventOutcomeApplication.appliedAt).toLocaleString() : "", followUpCount: followUps.recordCount ?? followUps.records?.length ?? 0, canReopen: true, session: cloneData(session) };
...
export function copyTravelV2DebugReport(session, options = {}) {
