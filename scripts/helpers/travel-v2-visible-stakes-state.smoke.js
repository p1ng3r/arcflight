...
export default async function runTravelV2VisibleStakesStateSmokeChecks() {
  ...
  const serialized = JSON.stringify(nonGmState.travelV2VisibleStakes);
  assert.equal(nonGmState.travelV2VisibleStakes.hasStakes, true, "non-GM runner state retains player-safe visible stakes");
  ...
  return { checked: ["runner-state-visible-stakes", "canonical-helper-output", "runner-state-input-immutability", "non-gm-player-safe-visible-stakes"] };
}
