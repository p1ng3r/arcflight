...
function baseReviewState(session, options = {}) {
  ...
  const title = publicSafe ? "Outcome Review" : "Final Outcome Package Review";
  const gmOnlyLabel = publicSafe ? "Restricted" : "GM-only";
  return deepFreeze({ version: TRAVEL_V2_EVENT_OUTCOME_PACKAGE_VERSION, title, status: outcomePackage.status, canPreparePackage: outcomePackage.canPreparePackage, blockedReasons: cloneData(outcomePackage.blockedReasons), hasBlockedReasons: outcomePackage.blockedReasons.length > 0, isCompleted: outcomePackage.isCompleted, alreadyApplied: outcomePackage.alreadyApplied, completedAt: outcomePackage.completedAt, eventRoundCount: outcomePackage.eventRoundCount, finalizedRoundCount: outcomePackage.finalizedRoundCount, eventOutcomeKey: outcomePackage.eventOutcomeKey, eventOutcomeLabel: outcomePackage.eventOutcomeLabel, summaryText: outcomePackage.summaryText, nextStepText: outcomePackage.nextStepText, reviewOnlyLabel: "Review only", pendingLabel: "Pending", alreadyAppliedLabel: "Already applied", deferredLabel: "Deferred to later PR", gmOnlyLabel, publicSafeLabel: "Public-safe", statusLabel, applyStatusLabel: outcomePackage.alreadyApplied ? "Already applied" : "Deferred to later PR", sections, roundSummaries: sections.rounds.entries, pressureSummary: sections.pressure, hazardSummary: sections.hazards.entries, shipScarCandidates: sections.shipScars.entries, fortuneCandidates: sections.fortunes.entries, rewardCandidates: sections.rewards.entries, consequenceCandidates: sections.consequences.entries, applyState: includeManagementDetails ? prepareTravelV2FinalOutcomeApplyState(session, options) : playerSafeApplyState(), managementDetails: includeManagementDetails ? { packageRecord: cloneData(outcomePackage.packageRecord), rawApplicationRecord: cloneData(session?.travelV2EventOutcomeApplication ?? null), marker: "GM-only management details" } : null });
}
...
function playerSafeApplyState(reason = "Only GMs can preview or apply final outcome ship updates.") { return deepFreeze({ isGM: false, available: false, canApply: false, disabled: true, disabledReason: reason, rows: [], records: [], changes: {} }); }
