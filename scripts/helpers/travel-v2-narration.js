...
function supportNarrationText(record = {}) {
  ...
  if (record.status === "used") return `${target} marked ${supporting}’s +${assist} assist used; no roll bonus is applied automatically.`;
  if (record.status === "dismissed") return `${supporting}’s Support assist for ${target} was dismissed.`;
  if (assist >= 2) return `${supporting}’s critical Support creates a +${assist} assist for ${target}.`;
  return `${supporting} supports ${target}, giving them a +${assist} assist to work with.`;
}
...
export function sanitizeTravelV2PublicNarration(narration) {
  ...
  return { roundNumber: source.roundNumber, title: source.title, completionState: source.completionState, complete, whatHappened: publicWhatHappened, publicWhatHappened, momentum: publicMomentum, support: publicSupport, focusBacklash: publicFocusBacklash, stationVignettes, stationOutcomeBullets: stationVignettes.map((v) => v.publicText), hazardNotes: { revealed: safeHazards }, suggestedReadAloud: publicSuggestedReadAloud, publicSuggestedReadAloud };
}
