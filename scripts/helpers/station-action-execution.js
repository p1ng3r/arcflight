...
import { getPf2eRollTotal, resolvePf2eActorStatistic, rollPf2eStatistic } from "./pf2e-statistics.js";
...
export function resolveAssignedActorStatistic(actor, statisticKey = "") {
  return resolvePf2eActorStatistic(actor, statisticKey);
}
...
export async function rollStationAction(shipActor, actionKey, options = {}) {
  ...
  if (statisticResolution.ok) {
    try {
      rollResult = await rollPf2eStatistic(assignedActor, statisticResolution.statistic, {
        event: options.event,
  ...
  const record = {
    ...
    rollStatus,
    total: getPf2eRollTotal(rollResult),
    degreeOfSuccess,
