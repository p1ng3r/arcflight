...
Arcflight includes a Travel Event Builder Foundation helper layer and a local GM-facing builder shell for future event authoring. The builder remains an authoring-only tool: saved custom drafts can be stored in the world-local `arcflight.travelEventBuilderLibrary` setting, but there is no drag/drop editor, compendium persistence, AI generation, automatic combat start, AP/RAP travel spending, automatic staged-effect application, or ship-resource mutation.
...
Future GM builder UI work should consume this helper layer instead of inventing separate event data shapes. The public helper exports are available on both `game.arcflight` and `game.arcflight.devTools`, including `createTravelEventDraft`, `normalizeTravelEventDraft`, `validateTravelEventDraft`, `finalizeTravelEventDraft`, `cloneTravelEventToDraft`, staged resource-effect helpers, round/prompt/outcome skeleton helpers, `prepareTravelEventBuilderFormOptions`, `applyTravelEventBuilderFormDataToDraft`, `prepareTravelEventBuilderRoundEditorState`, `applyTravelEventBuilderRoundFormDataToDraft`, `prepareTravelEventBuilderFinalOutcomeEditorState`, `prepareTravelEventBuilderFinalOutcomeEffectEditorState`, `applyTravelEventBuilderFinalOutcomeFormDataToDraft`, `applyTravelEventBuilderFinalOutcomeEffectFormDataToDraft`, `prepareTravelEventBuilderPreview`, `analyzeTravelEventBuilderQuality`, `prepareTravelEventBuilderQualityReport`, `getTravelEventBuilderLibrary`, `saveTravelEventBuilderDraftToLibrary`, `loadTravelEventBuilderDraftFromLibrary`, `deleteTravelEventBuilderDraftFromLibrary`, `duplicateTravelEventBuilderLibraryDraft`, and `prepareTravelEventBuilderLibraryState`.

Open the current builder with `game.arcflight.openTravelEventBuilder()`. It provides a Saved Drafts Library panel for explicit save/load/duplicate/delete/refresh actions, basic form controls for top-level draft fields (`key`, `name`, `category`, `baseDC`, `roundCount`, description/GM summary, tags, active resources, and Travel Five stations), a local Round Editor MVP for opening vignettes, per-round active stations, and active-station `playerAction` text, read-only outcome branch summaries, a local Final Outcomes Text Editor MVP for the five canonical outcomes (`criticalSuccess`, `success`, `mixed`, `failure`, and `criticalFailure`), a local Final Outcome Resource Effects Editor MVP for adding, editing, and removing supported proposed resource effects while preserving unsupported effects as read-only summaries, a Quality / Polish Gate panel with grouped errors, warnings, suggestions, checklist areas, readiness state, and score, the raw JSON editor as an advanced fallback, and pasted JSON import plus draft/final JSON export without writing compendia, actors, chat, or travel state.
...
During an active event, the runner displays the current event header, round title and opening vignette, round/event totals, Travel Five station prompts, assigned actor readouts, suggested statistic selectors, optional PF2E assigned-actor roll buttons, resource-option text, duplicate-result hints, a persistent **Travel Event Log**, staged round effects, final staged effects when available, and combat-handoff notes when backend state marks a handoff as available.
...
Station result feedback now persists in the runner: each recorded station result stores available `playerAction` prose and degree-based `rollFeedback`, keeps the existing inline station-card feedback, and adds the same narrative text to the **Travel Event Log** as a round-ordered record. Temporary notification quips remain a convenience after manual recording or PF2E assigned-actor rolls, but the runner does not automatically post these results to chat. When no event is active, the most recent completed event can show a compact **Last Event Log** beneath the completed-event summary.
