...
Arcflight includes a Travel Event Builder Foundation helper layer and a local GM-facing builder shell for future event authoring. The builder remains local-only and in-memory: there is no drag/drop editor, compendium persistence, AI generation, automatic combat start, AP/RAP travel spending, automatic staged-effect application, or ship-resource mutation.
