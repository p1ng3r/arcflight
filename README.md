...
- **Weapon** — weapon source components with starter entries, size/family/reload/damage profile schema, backend install/remove helpers, hull weapon mount validation, ship-owned installed weapon snapshots, mount occupancy tracking, validation preview support, and lifecycle install-state records. Weapon UI and combat firing remain future work.
